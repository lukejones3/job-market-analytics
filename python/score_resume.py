...
def resume_to_skills(cur, resume_id: str, resume_text: str, alias_patterns) -> int:
    ...
    # Tokenize section lines -> candidate tokens
    section_tokens = []
    ...
    # Build token variants to improve recall (punctuation/space-insensitive)
    def _variants(x: str):
        t = normalize_for_matching(x)
        out = set()
        out.add(t)
        ...
        # drop trailing periods (e.g. "sr." aliases if you add later)
        out.add(t.replace(".", ""))
    ...
    # Bulk map variants -> skill_id in one query
    alias_map = {}
    if variant_set:
        cur.execute("""
    ...
    # Apply mapping (prefer earlier/better variants)
    for tok in section_tokens:
        sid = None
...
def compute_confidence_score(resume_text: str, skills_found: int, market_jobs: int, matched_jobs: int,
                             sal_min, sal_max, skill_source_counts: Optional[dict] = None):
    """
    ...
    ssc = (skill_source_counts or {})
    n_alias = int(ssc.get('alias', 0) or 0)
    n_section = int(ssc.get('section', 0) or 0)
    n_heur = int(ssc.get('heuristic', 0) or 0)
    n_total = max(1, n_alias + n_section + n_heur)
    frac_heur = n_heur / float(n_total)
    frac_good = (n_alias + n_section) / float(n_total)
    if frac_heur >= 0.60:
        score -= 10
        flags.append('skills_mostly_heuristic')
    elif frac_good >= 0.60:
        score += 3  # small bump; cap below will apply
        flags.append('skills_mostly_high_confidence_sources')
    ...
    with db_conn() as conn:
        ...
        try:
            with conn.cursor(cursor_factory=DictCursor) as cur:
                ...
                # source breakdown of extracted skills (alias/section/heuristic)
                cur.execute("""
                  ...
                  GROUP BY source
                """, (resume_id,))
                skill_source_counts = {r["source"]: int(r["n"]) for r in cur.fetchall()}

                conf_score, conf_flags = compute_confidence_score(
                    resume_text=resume_text,
                    skills_found=len(resume_skill_ids),
                    skill_source_counts=skill_source_counts,
